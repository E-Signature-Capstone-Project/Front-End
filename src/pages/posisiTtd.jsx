...
export default function PosisiTtd() {
  ...
  const handleTempelTandaTangan = async () => {
    ...
    try {
      ...
      // ✅ Jika external signature (requested document)
      if (isRequestedDoc === "true" && requestId) {
        let baseline_id = localStorage.getItem("selectedBaselineId");


        // ✅ Auto-fetch baseline jika tidak ada di localStorage
        if (!baseline_id) {
          console.log("⚠️ No baseline_id in localStorage, fetching from API...");
          ...
          if (!baselineResponse.ok) {
            throw new Error("Tidak dapat mengambil data baseline. Pastikan Anda sudah membuat baseline signature.");
          }


          const baselinesData = await baselineResponse.json();
          console.log("📊 Baselines response:", baselinesData);


          // Handle jika response adalah object dengan property 'baselines'
          const baselines = Array.isArray(baselinesData) 
            ? baselinesData 
            : (baselinesData.baselines || []);
          
          if (!baselines || baselines.length === 0) {
            throw new Error("Anda belum memiliki baseline signature. Silakan buat baseline terlebih dahulu di menu Baseline Sign.");
          }


          // Ambil baseline pertama
          baseline_id = baselines[0].baseline_id;
          console.log("✅ Using first baseline ID:", baseline_id);
        }


        console.log("📤 Sending external signature request with baseline_id:", baseline_id);
