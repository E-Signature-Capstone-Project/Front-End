...
export default function IncomingRequestsTab() {
  ...
  async function fetchRequests() {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/requests/incoming`, {
        method: 'GET',
        headers: { 
  ...
  const handleViewDetail = (request) => {
    const senderName = request.requester?.name || 'Pengirim';
    const senderEmail = request.requester?.email || request.recipient_email;
    const note = request.note || '<em class="text-gray-400">Tidak ada catatan</em>';
    const status = getStatusText(request.status);
    const statusColor = request.status === 'approved' || request.status === 'completed' ? 'text-green-600' : 
      ...
      html: `
        <div class="text-left space-y-3">
          ...
          <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
            ...
            <p class="font-semibold text-blue-900">${senderName}</p>
            <p class="text-sm text-blue-700 mt-1">📧 ${senderEmail}</p>
          </div>
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            <p class="text-xs text-gray-500 mb-1">Waktu Diterima</p>
  ...
  function getStatusColor(status) {
    ...
    if (val === "approved" || val === "completed" || val === "selesai") return "bg-green-500 text-white";
  ...
  function getStatusText(status) {
    if (!status) return "-";
    const val = status.toLowerCase();
    if (val === "approved" || val === "completed") return "Disetujui";
  ...
  if (loading) {
    return <p className="text-gray-500 text-center py-8">Memuat data...</p>;
  }
              ...
              <tr key={r.request_id || idx} className="text-center hover:bg-gray-50">
                <td className="border px-5 py-3">
                  <div className="text-sm">
                    ...
                    <p className="text-xs text-gray-500">{r.requester?.email || r.recipient_email}</p>
                  </div>
                </td>
